...
2. 筛选条目：结合主题相关性、原文内容和社区热度，选出最多 5 条技术精选和最多 2 条圈外探索。项目分享、工具推荐或实践经验征集帖先读取有界 HN 评论样本；只有能介绍具体内容时才参与最终入选，评论不足或处理失败时由其他合格候选递补
3. 生成摘要：获取入选内容的原文，并结合网页提供的介绍生成中文摘要；同一次摘要调用会判断材料是否足够。原文被验证页阻止、且既有抓取路径失败时，先搜索有明确转载关系的同稿页面，再尝试既有替代报道路径。恢复失败或材料不足时补充 HN 讨论；已有页面材料会保留，并与评论分别标注，讨论也不可用时明确提示，不根据标题或模型常识补写
4. 输出与发布：生成用于阅读、发布和复盘的文件；发布作为独立步骤执行

征集帖使用独立的摘要 prompt，输出一句帖子介绍和两到三个分条项目、工具或实践经验，并标注“根据 Hacker News 部分评论：”。每条先说是什么、解决什么问题，再保留一个有辨识度的特点；Markdown 和发布 JSON 保留分条换行。评论采样优先覆盖不同顶层回答，不代表整个讨论区。系统不会把所有 `Ask HN` 都视为征集帖；本身已有实质内容的自帖和普通短公告仍按原材料生成摘要。

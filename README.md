...
3. 生成摘要：获取入选内容的原文，并结合网页提供的介绍生成中文摘要；长正文先选取与当前条目相关的段落及邻近上下文；同一次摘要调用会判断这些材料是否足以介绍当前条目，短公告也可以足够。原文被验证页阻止、且既有抓取路径失败时，先搜索有明确转载关系的同稿页面，再尝试既有替代报道路径。恢复失败或材料不足时补充 HN 讨论；已有页面材料会保留，并与评论分别标注，讨论也不可用时明确提示，不根据标题或模型常识补写

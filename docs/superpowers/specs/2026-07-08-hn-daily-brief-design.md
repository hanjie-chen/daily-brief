...
排序结果仍需满足 section 的数量限制：AI section 最多 5 条，Non-AI Hot section 最多 2 条。
...
JSON 需要包含所有候选 stories，包括最终未入选的条目。保留字段包括 source、HN item id、title、source URL、HN discussion URL、created_at、points、comments、matched keywords、score、selected、section 和 rejection reason。这样可以在 HN points/comments 变化后仍复盘当天快照，并用历史候选数据调试关键词和权重。
...
- 原始候选 JSON 包含入选和未入选 stories，并记录 score、section 和 rejection reason。

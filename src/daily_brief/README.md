...
7. Remaining HN-hot candidates are ordered by points and comments. `cli.py` walks at most five candidates, retrieves their article material, and asks `topic_classifier.py` for one of four decisions: `ai`, `core_non_ai`, `outside`, or `uncertain`. Only a positive `outside` decision can occupy one of the two exploration slots; retrieval failure is recorded as `topic_unknown`, while a model failure after successful retrieval is `classifier_failed`. Both failures are excluded from exploration. The walk stops once both slots are filled or the five-candidate bound is reached.

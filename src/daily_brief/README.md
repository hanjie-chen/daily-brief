...
- Jina Reader is an anonymous retrieval fallback only for direct responses explicitly marked `cf-mitigated: challenge`, or for direct HTML responses whose `trafilatura` extractor returns `empty_content`. Each item gets at most one Jina attempt. The request uses the same timeout and extracted-response size bound as direct retrieval, accepts at most a five-minute cached result, and validates the bounded JSON envelope, provider status, origin status and URL, and non-empty content before accepting it. Ordinary HTTP errors, extraction exceptions, other content types, PDF failures, GitHub API failures, and GitHub raw failures remain terminal. There is no separate login-wall or paywall detector: a `200` HTML wall that `trafilatura` reduces to empty is indistinguishable from a CSR shell and can therefore trigger this fallback.

...
- GitHub repository-root URLs use the public GitHub README API without authentication and report `github_readme` provenance. The adapter must remain limited to root repository URLs; blob, tree, release, issue, pull request, Gist, and other GitHub URLs must not be silently replaced with repository README content. GitHub API failures do not fall back to Jina.

...
- Jina Reader is an anonymous retrieval fallback only for direct responses explicitly marked `cf-mitigated: challenge`, direct HTML responses whose `trafilatura` extractor returns `empty_content`, or a structured `SSLCertVerificationError` with OpenSSL verify code 20 (`unable to get local issuer certificate`). Each item gets at most one Jina attempt. The request uses the same timeout and extracted-response size bound as direct retrieval, accepts at most a five-minute cached result, and validates the bounded JSON envelope, provider status, origin status and URL, and non-empty content before accepting it. Other TLS verification failures—including expired, hostname-mismatched, and self-signed certificates—remain terminal, as do ordinary HTTP errors, extraction exceptions, other content types, PDF failures, GitHub API failures, and GitHub raw failures. There is no separate login-wall or paywall detector: a `200` HTML wall that `trafilatura` reduces to empty is indistinguishable from a CSR shell and can therefore trigger this fallback.
...
- Every successfully downloaded HTML response is processed by `trafilatura` with comments disabled and precision favored. The HTML download bound is 4 MiB and the extracted UTF-8 text bound is 256 KiB. An extraction exception is terminal and full-page visible text is never used; only a normal empty `trafilatura` result on the direct transport can trigger Jina.
